...
for Indian commodities and mandi centers across 365+ days.
"""
...
CROP_PROFILES: Dict[str, Dict] = {
    ...
    "Onion": {
        ...
        "lean_months": [8, 9, 10],  # Late monsoon / pre-Kharif price strength
        "msp": 0.0,
        "trend_drift": 0.05,
    },
    "Potato": {
    ...
    for center in centers:
        ...
        for d in range(days):
            ...
            if month in profile["peak_months"]:
                arrival_mult = random.uniform(1.4, 2.0)
                seasonal_price_factor = random.uniform(0.94, 0.99)
            elif month in profile["lean_months"]:
                arrival_mult = random.uniform(0.5, 0.8)
                seasonal_price_factor = random.uniform(1.03, 1.14)
            else:
            ...
            if day_of_week == 6:
                ...
                arrival_tonnes = profile["base_arrival"] * arrival_mult * random.uniform(0.90, 1.10)

            mean_rev_pull = (profile["base_price"] - current_price) * 0.03
            shock = np.random.normal(0, profile["price_volatility"] * 0.35)
