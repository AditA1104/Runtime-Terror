...
import { getLocalizedMandiName, translations } from './translations';
...
export function formatNotificationMessage(notif: NotificationItem, lang: SupportedLang): string {
  ...
  if (notif.templateType) {
    ...
    const locCenter = getLocalizedMandiName(center, lang);
    const locCrop = crop ? (dict['crop_' + crop.toLowerCase()] || crop) : (lang === 'kn' ? 'ಬೆಳೆ' : lang === 'hi' ? 'फसल' : lang === 'mr' ? 'पीक' : 'Crop');

    switch (notif.templateType) {
      case 'BOOKED':
        switch (lang) {
          case 'kn': return `AgriQ: ಸ್ಲಾಟ್ ದೃಢೀಕರಿಸಲಾಗಿದೆ! ${locCenter}ಯಲ್ಲಿ ${locCrop}ಗಾಗಿ ಟೋಕನ್ ${token || ''} ರಚಿಸಲಾಗಿದೆ. ಸರತಿ ಸ್ಥಾನ: #${queuePos || 1}. QR ಪಾಸ್ ಸಿದ್ಧವಾಗಿಡಿ.`;
          case 'hi': return `AgriQ: स्लॉट कन्फर्म हुआ! ${locCenter} में ${locCrop} के लिए टोकन ${token || ''} जारी किया गया। कतार स्थिति: #${queuePos || 1}। क्यूआर पास तैयार रखें।`;
          case 'mr': return `AgriQ: स्लॉट निश्चित झाला! ${locCenter} येथे ${locCrop} साठी टोकन ${token || ''} तयार करण्यात आले. रांग क्रमांक: #${queuePos || 1}. क्यूआर पास तयार ठेवा.`;
          case 'te': return `AgriQ: స్లాట్ ఖరారైంది! ${locCenter} లో ${locCrop} కోసం టోకెన్ ${token || ''} జారీ చేయబడింది. క్యూ స్థానం: #${queuePos || 1}.`;
          case 'pa': return `AgriQ: ਸਲਾਟ ਪੱਕਾ! ${locCenter} ਵਿਖੇ ${locCrop} ਲਈ ਟੋਕਨ ${token || ''} ਜਾਰੀ ਕੀਤਾ ਗਿਆ। ਲਾਈਨ ਨੰਬਰ: #${queuePos || 1}।`;
      ...
      case 'CHECKED_IN':
        switch (lang) {
          case 'kn': return `AgriQ: ${locCenter} ಗೇಟ್ ಚೆಕ್-ಇನ್ ಪೂರ್ಣಗೊಂಡಿದೆ. ಧರ್ಮಕಾಟಾ (ವೇಬ್ರಿಡ್ಜ್ ಲೇನ್ 2) ಗೆ ತೆರಳಿ.`;
          case 'hi': return `AgriQ: ${locCenter} पर चेक-इन पूरा हुआ। धर्मकांटा (वेब्रिज लेन 2) पर जाएं।`;
          case 'mr': return `AgriQ: ${locCenter} येथे प्रवेश चेक-इन पूर्ण. वजन काटा (वेब्रिज लेन २) कडे जा.`;
          case 'te': return `AgriQ: ${locCenter} గేట్ చెక్-ఇన్ పూర్తయింది. వేబ్రిడ్జ్ లేన్ 2 కి వెళ్ళండి.`;
          case 'pa': return `AgriQ: ${locCenter} ਗੇਟ ਤੇ ਚੈੱਕ-ਇਨ ਪੂਰਾ ਹੋਇਆ। ਕੰਡੇ (ਵੇਬ੍ਰਿਜ ਲਾਈਨ 2) ਤੇ ਜਾਓ।`;
